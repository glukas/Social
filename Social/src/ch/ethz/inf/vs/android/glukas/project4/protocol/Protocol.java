...
public class Protocol implements ProtocolDelegate, SecureChannelDelegate, MessageRelayDelegate {
	...
	public void getUserWall(String distUsername) throws NetworkException {
		...
		PublicHeader header = new PublicHeader(localUserId, listUserId.get(0), ConsistencyByte.DATA_SEARCH.getState(), 0);
		secureChannel.sendMessage(JSONObjectFactory.createJSONObject(msg, 0).toString(), header);
	}
	...
	public void onMessageReceived(String message, PublicHeader header) {
		...
		Message msg = MessageParser.parseMessage(message, header);
